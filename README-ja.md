...
これで全 12 コマンド（[コマンド一覧](#コマンド一覧)を参照）が自分のサーバーにだけ登録されます。
すべてのコマンドは既定でサーバー管理者以外から隠されます（`default_member_permissions`）。
他のロールやメンバーへの許可は、登録後に「サーバー設定 → 連携サービス」で追加してください。
...
`/start` は ECS サービスを起動します。
残る 11 コマンドは Palworld 公式の REST API（HTTP ベースの API）を経由し、サーバー稼働中のみ動作します。
サーバー停止中に実行すると、停止中である旨を返信します。
すべてのコマンドは既定で管理者専用です。

| コマンド | 動作 |
|---|---|
| `/start` | オンデマンドサーバーを起動する |
| `/info` | サーバー名・バージョン・説明を表示する |
| `/players` | 現在オンラインのプレイヤーを一覧表示する |
| `/settings` | サーバー設定を表示する |
| `/metrics` | サーバーの FPS・稼働時間・接続人数を表示する |
| `/announce <message>` | ゲーム内に全体メッセージを送る |
| `/kick <userid> [message]` | Palworld ユーザー ID でプレイヤーを追放する |
| `/ban <userid> [message]` | Palworld ユーザー ID でプレイヤーを BAN する |
| `/unban <userid>` | BAN を解除する |
| `/save` | ワールドを即時保存する |
| `/shutdown [waittime] [message]` | 猶予時間をおいてシャットダウンする |
| `/stop` | サーバーを即時に強制停止する |

`userid` は Palworld のユーザー ID です（例: `steam_0123456789ABCDEF`）。
...
- すべてのコマンドは三層で保護されています。コマンドは自分のサーバーだけに登録され、既定ではサーバー管理者しか実行できず（許可の追加は「サーバー設定 → 連携サービス」）、さらに Lambda が Discord の Ed25519 署名とギルド ID を検証します。Function URL は公開されていますが、Discord 以外からのリクエストはすべて拒否されます。
...
- 管理用の REST API（HTTP ベースの管理用 API）が待ち受けるポート 8212 は、インターネットへは一切公開されません。到達経路は VPC 内部のプロキシ Lambda だけです。タスクのセキュリティグループはポート 8212 をそのプロキシ Lambda のセキュリティグループからのみ許可し、プロキシ Lambda には Function URL も外向き通信もありません。`ADMIN_PASSWORD` を保持するのはタスクとそのプロキシ Lambda だけです。
