...
- **Discord interactions Lambda** (`discord-interactions.ts` construct, code in `lambda/discord_interactions/`, Python, fixed name `palworld-discord-interactions`) with a public Function URL (exported as the `DiscordInteractionsEndpointUrl` output). It verifies the Ed25519 signature with `discord-interactions` (bundled at synth time via pip), checks the payload's `guild_id`, returns a deferred response within Discord's 3-second limit, then invokes itself asynchronously to do the slow work and post the follow-up message. The ECS service-control policy is attached to both the ECS task role and this Lambda's role. It handles the full command set: `/start` scales `desiredCount` to 1; the other commands (`/info`, `/players`, `/settings`, `/metrics`, `/announce`, `/kick`, `/ban`, `/unban`, `/save`, `/shutdown`, `/stop`) are backed by Palworld's official REST API and refuse when the server is stopped. This Lambda lives outside the VPC and cannot reach the task's private IP, so it never calls the REST API directly.
- **REST API proxy Lambda** (`rest-api-proxy.ts` construct, code in `lambda/rest_api_proxy/`, Python, standard library only so no pip bundling): a VPC-internal Lambda with no Function URL and no internet egress, placed in the VPC's isolated subnets. The receiver Lambda invokes it synchronously (`RequestResponse`) with `{private_ip, method, path, body}`; it relays a single Basic-auth HTTP call to the task's private `IP:8212` and returns the response as its return value. 8212 is opened in the service security group **only** to this Lambda's security group (never the internet), and the admin password lives only here. The task's private IP is published to the `/palworld/server/private-ip` SSM parameter by the watchdog on each startup, read by the receiver Lambda, and passed in the invoke payload — so the proxy needs no AWS API access and the design stays NAT-free and zero-cost at idle.
